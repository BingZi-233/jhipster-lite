...
import static tech.jhipster.lite.shared.slug.domain.JHLiteModuleSlug.SPRING_BOOT_CUCUMBER_MVC;
import static tech.jhipster.lite.shared.slug.domain.JHLiteModuleSlug.SPRING_BOOT_CUCUMBER_WEBFLUX;
...
class CucumberModuleConfiguration {
  ...
  JHipsterModuleResource cucumberWebfluxInitializationModule(CucumberApplicationService cucumber) {
    return JHipsterModuleResource.builder()
      ...
      .propertiesDefinition(JHipsterModulePropertiesDefinition.builder().addBasePackage().addIndentation().addProjectBaseName().build())
      .apiDoc(SPRING_BOOT_COMPONENT_TESTS_API_GROUP, "为项目添加Cucumber与Webflux的集成")
